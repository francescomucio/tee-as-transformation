...
MotherDuck is a cloud-hosted DuckDB service. You can initialize a project with MotherDuck using:

```bash
t4t init my_project -d motherduck
```

This will create a `project.toml` with the correct MotherDuck configuration. Alternatively, configure it manually using the `md:` or `motherduck:` connection string prefix:
...
Or using the legacy `[connection]` section (as generated by `t4t init -d motherduck`):
