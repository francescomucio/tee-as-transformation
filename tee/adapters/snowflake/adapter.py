...
class SnowflakeAdapter(DatabaseAdapter):
    ...
    def create_view(self, view_name: str, query: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        if metadata and (("schema" in metadata and metadata["schema"]) or ("description" in metadata and metadata["description"])):
            create_query = self._build_view_with_column_comments(qualified_view_name, query, metadata)
        else:
    ...
    def _build_view_with_column_comments(self, qualified_view_name: str, query: str, metadata: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            escaped_view_description = view_description.replace("'", "''") if view_description else ""
            ...
            view_comment_part = f" COMMENT='{escaped_view_description}'" if escaped_view_description else ""
            
            if not column_descriptions:
                # Only view comment, no column comments
                create_query = f"CREATE OR REPLACE VIEW {qualified_view_name}{view_comment_part} AS {query}"
            else:
